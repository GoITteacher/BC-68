...
function onFormSubmit(e) {
  e.preventDefault();
  const query = refs.form.elements.query.value.trim(); // 'BMW';
  const selected = refs.form.elements.options.value;

  const filteredCars = cars.filter(obj => {
    return obj.car === query; // false
  });
...
function carTemplate(car) {
  return `<li data-id="${car.id}" class="list-item">
  <img
    src="${car.img}"
    alt=""
  />
  <h4>${car.car} ${car.type}</h4>
  <p>Price: ${car.price}</p>
